...
| 用户注册（登录页新增入口） | 注册申请 | 待审核账号、审核状态、审核人 |

**结论：核心实体 8 个** —— 用户、地块、设备、传感器数据、告警阈值、告警、控制日志、注册申请。（智能问答走 RAG 知识库，不落在关系库里。）
...
### 8. 注册申请（register_request）— 管理员审核注册

| 属性 | 概念类型 | MySQL 类型 | 说明 |
|---|---|---|---|
| 申请ID | 整型 · PK | `BIGINT` 自增 | |
| 申请账号 | 字符串 | `VARCHAR(50)` | 待审核的登录账号 |
| 密码 | 字符串 | `VARCHAR(100)` | 存哈希；审核通过后写入 user 表 |
| 申请角色 | 枚举 | `VARCHAR(20)` | 用户自选：农户 / 农场管理员 / 系统管理员，审核时可调整 |
| 状态 | 枚举 | `VARCHAR(10)` | 待审核 / 已通过 / 已拒绝 |
| 拒绝原因 | 字符串 | `VARCHAR(200)` 可空 | 拒绝时填写 |
| 申请时间 | 日期时间 | `DATETIME` | |
| 审核时间 | 日期时间 | `DATETIME` 可空 | |
| 审核人 | 字符串 | `VARCHAR(50)` 可空 | |
| 关联用户ID | 整型 · FK | `BIGINT` 可空 | 通过后关联生成的 user.id |

> 审核流程：注册提交 → 插入 `待审核` 记录 → 管理员审核 → 通过则写入 `user` 表并回填 `user_id`；拒绝则填 `reject_reason`。
...
erDiagram
    ...
    REGISTER_REQUEST ||--o| USER : "审核通过后生成"
...
6. **注册审核**：`register_request` 是"待激活的用户"，通过后才写入 `user`，两者是 1:0..1（`user_id` 通过后回填）。
