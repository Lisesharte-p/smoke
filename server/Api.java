...
public class Api {
    ...
    private static Map<String, String> fetchQWeatherNow() throws IOException {
    ...
    private static List<Map<String, String>> fetchQWeatherDaily() throws IOException {
    ...
    private static String iconEmoji(String code) {
        ...
        if (c == 100) return "☀️";                     // 晴
        if (c == 101 || c == 102 || c == 103) return "⛅"; // 多云 / 少云 / 晴间多云
        if (c == 104) return "☁️";                     // 阴
        if (c >= 300 && c <= 399) return "🌧️";         // 雨
        if (c >= 400 && c <= 499) return "❄️";         // 雪
        if (c >= 500 && c <= 515) return "🌫️";         // 雾 / 霾
        return "🌤️";
    }
