...
#define WIFI_CONNECT_MAX_ATTEMPTS 10       /* 扫描+连接最大尝试次数 */
...
int WifiConnect(const char *ssid, const char *psk)
{
    ...
    // 扫描 + 连接热点,失败自动重新扫描重试,避免找不到热点时死循环
    int connect_success = 0;
    for (int attempt = 0; attempt < WIFI_CONNECT_MAX_ATTEMPTS && connect_success == 0; attempt++) {
        // 轮询查找WiFi列表
        g_staScanSuccess = 0;
        ...
        if (connect_success == 0) {
            printf("attempt %d: no wifi matched, rescan in 3s...\r\n", attempt + 1);
            sleep(3);
        }
    }
    if (connect_success == 0) {
        printf("ERROR: connect wifi failed after %d attempts\r\n", WIFI_CONNECT_MAX_ATTEMPTS);
        return -1;
    }
